...
## 2026-06-23 - [Descriptive Placeholders in Forms]
**Learning:** Adding descriptive `placeholder` attributes (e.g., 'e.g. Jane Doe') significantly improves form clarity by showing expected formats, while using Tailwind classes like `placeholder:text-[color]/30` ensures they remain visually distinct from user input.
**Action:** Always include descriptive `placeholder` attributes for form inputs and textareas, styled with appropriate contrast constraints to maintain visual hierarchy.
