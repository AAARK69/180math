...
## 2024-06-07 - Ensure Focus Visible on Custom Buttons
**Learning:** Interactive custom button components, such as elements utilizing the `.btn-glow` class, frequently lack proper keyboard navigation indicators because default outlines are often suppressed by reset stylesheets or custom styling.
**Action:** Always ensure that any custom interactive elements (like `.btn-glow` buttons) have explicit `focus-visible` utility classes (e.g., `focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E5BE85]`) applied to maintain keyboard navigation flow and accessibility.
