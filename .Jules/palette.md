...
## 2024-08-27 - [Mobile CSS Transform/Opacity Logic]
**Learning:** The `.reveal` animation class, which relies on an IntersectionObserver to change opacity from 0 to 1 as elements scroll into view, causes critical content to remain invisible by default on mobile devices if the observer fails or viewport constraints delay firing.
**Action:** The `.reveal` animation class must default to fully visible (`opacity: 1`, `transform: none`) on all elements to ensure content is seen by default on mobile devices. The hidden initial state (`opacity: 0`, transform offsets) should be restricted to desktop viewports via `@media (min-width: 768px)` to prevent content from remaining permanently invisible before scrolling on small screens.
